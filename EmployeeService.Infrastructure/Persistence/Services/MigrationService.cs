...
public class MigrationService(ILogger<MigrationService> logger, IServiceProvider serviceProvider)
{
    ...
    private async Task ApplyMigrationsWithRetryAsync(int maxRetries, int retryDelaySeconds,
    ...
    private async Task<bool> TryApplyMigrationsAsync(int retryCount, int maxRetries,
        CancellationToken cancellationToken)
    {
        try
        {
